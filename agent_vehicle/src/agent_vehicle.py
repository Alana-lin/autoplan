...
class CarlaAgentVehicle(object):
    def __init__(self):
        rospy.init_node('agent_vehicle', anonymous=True)
        ...
        if spawn_point_param and rospy.get_param('~spawn_agent_vehicle'):
    ...
    def start(self):
        ...
        # Spawn the vehicle.
        if not rospy.get_param('~spawn_agent_vehicle'):
            ...
            if self.actor_spawnpoint:
                ...
                spectator.set_transform(carla.Transform(transform.location + carla.Location(z=60),carla.Rotation(pitch=-90,yaw = 90)))
    ...
    def run(self):
        ...
        rospy.loginfo("CARLA world available. Spawn agent vehicle...")
...
def main():
    agent_vehicle = CarlaAgentVehicle()
    try:
        agent_vehicle.run()
    finally:
